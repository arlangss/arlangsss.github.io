...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id) {
        // Update data
        $queryStr = ($jenis === 'keluar') ?
            "UPDATE pea SET nama_barang=?, jumlah=?, jenis=?, nama_pengambil=?, waktu=NOW() WHERE id=?" :
            "UPDATE pea SET nama_barang=?, jumlah=?, jenis=?, nama_pengambil=? WHERE id=?";
        $typesStr = ($jenis === 'keluar') ? "sissi" : "sissi";
        query($koneksi, $queryStr, $typesStr, $nama_barang, $jumlah, $jenis, $nama_pengambil, $id);
    } else {
        // Insert data baru
        $queryStr = ($jenis === 'keluar') ?
            "INSERT INTO pea (nama_barang, jumlah, jenis, nama_pengambil, waktu) VALUES (?, ?, ?, ?, NOW())" :
            "INSERT INTO pea (nama_barang, jumlah, jenis, nama_pengambil) VALUES (?, ?, ?, ?)";
        $typesStr = "siss";
        query($koneksi, $queryStr, $typesStr, $nama_barang, $jumlah, $jenis, $nama_pengambil);
    }
    ...
    <form method="post">
        ...
        <select name="nama_barang" required>
            ...
            <?php foreach ($stok_awal as $nama => $stok): ?>
                <option value="<?= $nama ?>" <?= ($edit_data['nama_barang'] ?? '') === $nama ? 'selected' : '' ?>><?= $nama ?></option>
            <?php endforeach; ?>
        </select>
        <input type="number" name="jumlah" placeholder="Jumlah" required value="<?= $edit_data['jumlah'] ?? '' ?>">
        <select name="jenis" required>
            <option value="masuk" <?= ($edit_data['jenis'] ?? '') === 'masuk' ? 'selected' : '' ?>>Masuk</option>
            <option value="keluar" <?= ($edit_data['jenis'] ?? '') === 'keluar' ? 'selected' : '' ?>>Keluar</option>
        </select>
